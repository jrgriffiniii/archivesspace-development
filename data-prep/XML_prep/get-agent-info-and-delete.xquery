...
declare variable $agents as document-node()* := doc("file:/Users/heberleinr/Documents/SVN_Working_Copies/trunk/rbscXSL/ASpace%20tools/names_A-F.xml");
	...
	for $agent-link in $agents//cid[not(.='') and . = $cid]
		...
		return 
			...
			insert nodes 
				...
				<rules>{if($field/data(@rules)) then $field/data(@rules) else 'dacs'}</rules>,
				<source>{if($field/data(@source)) then $field/data(@source) else 'local'}</source>
				)
			as last into $agent-link/..,
			if (every $name in $field/../* satisfies contains(normalize-space($name), normalize-space($agent-link/../name-old)))
			then delete node $field/..
			else delete node $field[contains(normalize-space(.), normalize-space($agent-link/../name-old))], 
			if($field[ead:ptr])
			then delete node $field[contains(normalize-space(.), normalize-space($agent-link/../name-old))]/parent::ead:origination/../../ead:bioghist[@id=$field/ead:ptr/@target]
			else if ($field/parent::ead:origination[../../ead:bioghist 
				and (count(../../ead:bioghist) = count(../ead:origination/*))]
				and (every $name in $field/parent::ead:origination/* satisfies contains(normalize-space($name), normalize-space($agent-link/../name-old)))
				)
			then 
